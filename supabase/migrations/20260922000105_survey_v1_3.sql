-- Version 1.3 replaces Nike-owned sub-brands, clarifies four signal scales,
-- and adds five Then & Now choices. Historical rows keep their own schema.
begin;
alter table public.survey_responses drop constraint survey_version_supported;
alter table public.survey_responses add constraint survey_version_supported check (survey_version in ('1.0','1.1','1.2','1.3'));
alter table public.survey_responses add column attribute_audience integer check (attribute_audience between 0 and 10);
alter table public.survey_responses add column attribute_focus integer check (attribute_focus between 0 and 10);
alter table public.survey_responses add constraint v13_required_fields check (
  survey_version <> '1.3' or (
    ...
    and attribute_focus is not null
    and attribute_lifestyle is null and attribute_for_athletes is null
    and attribute_for_normal_people is null and attribute_product_led is null
    and attribute_culture_led is null
  )
);
alter table public.survey_responses add constraint v13_feeling_choices check (
  survey_version <> '1.3' or (
    nike_used_to_feel = any(array['Bold','Innovative','Performance-focused','Style-driven','Accessible','Predictable','Out of touch','Confident','Creative','Premium','Everyday','Inspiring']::text[])
...
begin
  foreach field in array array['interest_sport','interest_sneakers','interest_fashion','interest_pop_culture','overall_perception','perception_change','culture_score','product_innovation_score'] loop
    ...
    execute format('alter table public.survey_responses add constraint %I check ((survey_version in (''1.2'',''1.3'') and %I between 0 and 10) or (survey_version in (''1.0'',''1.1'') and %I between 1 and %s))', field || '_scale_by_version', field, field, legacy_max);
  end loop;
  foreach field in array array['attribute_performance','attribute_innovation'] loop
    execute format('alter table public.survey_responses drop constraint %I', field || '_scale_by_version');
    execute format('alter table public.survey_responses add constraint %I check ((survey_version = ''1.0'' and %I is null) or (survey_version = ''1.1'' and %I between 1 and 7) or (survey_version in (''1.2'',''1.3'') and %I between 0 and 10))', field || '_scale_by_version', field, field, field);
  end loop;
  foreach field in array array['attribute_lifestyle','attribute_for_athletes','attribute_for_normal_people','attribute_product_led','attribute_culture_led'] loop
    execute format('alter table public.survey_responses drop constraint %I', field || '_scale_by_version');
    execute format('alter table public.survey_responses add constraint %I check ((survey_version in (''1.0'',''1.3'') and %I is null) or (survey_version = ''1.1'' and %I between 1 and 7) or (survey_version = ''1.2'' and %I between 0 and 10))', field || '_scale_by_version', field, field, field);
  end loop;
end $$;
...
alter table public.survey_responses add constraint reasons_when_changed check (
  (survey_version in ('1.2','1.3') and ((perception_change = 5 and jsonb_array_length(perception_change_reasons) = 0) or (perception_change <> 5 and jsonb_array_length(perception_change_reasons) between 1 and 10)))
...
alter table public.brand_tier_rankings drop constraint brand_tier_rankings_brand_v11_check;
alter table public.brand_tier_rankings add constraint brand_tier_rankings_brand_v13_check check (brand = any(array['Nike','adidas','New Balance','ASICS','Salomon','On','Hoka','Puma','Jordan','Converse','Vans','Reebok','Saucony','Under Armour']::text[]));
...
begin
 if jsonb_typeof(payload->'brand_tiers') <> 'object' or
    (select count(*) from jsonb_object_keys(payload->'brand_tiers')) <> (case payload->>'survey_version' when '1.3' then 10 when '1.2' then 10 when '1.1' then 13 else 8 end) then
   raise exception 'Exactly % brand rankings required', (case payload->>'survey_version' when '1.3' then 10 when '1.2' then 10 when '1.1' then 13 else 8 end);
 end if;
